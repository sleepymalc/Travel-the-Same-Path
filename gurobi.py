...
def solve_euclidian_tsp(points, threads=0, timeout=None, gap=None):
    """
    Solves the Euclidean TSP problem to optimality using the MIP formulation 
    ...
    dist = {(i, j): math.sqrt(sum((points[i][k] - points[j][k])**2 for k in range(2))) for i in range(n)
            for j in range(i)}
